...
local small_time = vim.fn.has('win32') == 1 and 75 or 15
...
T['as_colorscheme() methods']['write()']['makes unique color scheme name'] = function()
  ...
  if child.fn.has('win32') == 1 then MiniTest.skip('`vim.fn.strftime()` does not work on Windows') end
...
T['animate()']['works'] = function()
  if child.fn.has('win32') == 1 then MiniTest.skip('Too flaky on Windows') end
...
T['animate()']['respects `opts.transition_steps`'] = function()
  ...
  sleep(0.5 * default_transition_duration - small_time - 10)
  eq(is_cs_1(), true)
